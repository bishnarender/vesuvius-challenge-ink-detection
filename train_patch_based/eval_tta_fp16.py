...
def main(args):
    ...
    for i, valid_idx in enumerate(fragment_ids):
        ...
        dataloader = InkDetDataModule(
            ...
            preprocess_in_model=False, # True, my_ False
        ).test_dataloader()
        ...
        model = InkDetLightningModel.load_from_checkpoint(
            ...
            pretrained=False,
            preprocess_in_model=False, # my_ False
        )
        model.eval()
        model = model.half().to(device=device)
        y_valid = np.array(
            Image.open(
                f"vesuvius-challenge-ink-detection-5fold/train/{valid_idx}/inklabels.png"
            ).convert("1")
        )
        ...
        for batch in tqdm(dataloader):
            ...
            pad = (MAX_IMG_SIZE - args.image_size) // 2
            if pad > 0:
        ...
        fragment_mask = (
            np.array(
                Image.open(
                    f"vesuvius-challenge-ink-detection-5fold/train/{valid_idx}/mask.png"
                ).convert("1")
            )
            > 0
        )
