...
# https://github.com/yukke42/kaggle-vesuvius-challenge-ink-detection/blob/main/inkdet/tools/preprocess.py
...
@logger.catch()
def main():
    root_path = Path("input/train")
    in_split: str = "2"
    in_path = root_path / in_split
    # in_path => input/train/2
    ...
    for filename in ["inklabels.png", "ir.png", "mask.png"]:
        ...
        # cv2.IMREAD_UNCHANGED => return the loaded image as is.
        image = cv2.imread(str(in_path / filename), cv2.IMREAD_UNCHANGED)
        # image.shape (inklabels.png) => (14830, 9506, 3) i.e., (H, W, C)
        
        if image.ndim == 2:
            _, w = image.shape
        elif image.ndim == 3:
            _, w, _ = image.shape
            image = image[..., 0] # keep only first channel. 
        else:
            raise ValueError(f"Unexpected image shape: {image}")
        
        # .imwrite(filename, img[, params]) => saves the image to the specified file. image format is chosen based on the filename extension.
        # image[:, : w // 2].shape (inklabels.png) => (14830, 4753)
        # image[:, w // 2 :].shape (inklabels.png) => (14830, 4753)
    ...
    for in_tiff_file in sorted((in_path / "surface_volume").glob("*.tif")): # tif => tag image format.
        
        logger.info(f"Processing {in_tiff_file.name} ...")
        image = cv2.imread(str(in_tiff_file), cv2.IMREAD_UNCHANGED)
        _, w = image.shape
        # image.shape => (14830, 9506)
