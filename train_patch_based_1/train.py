# This file is similar to "train_patch_based/train.py" so common parts has been explained in "train_patch_based/train.py".
...
def grid_cutout(
    volume, label, max_height: int = 2, max_width: int = 2, prob: float = 0.5
):
    if np.random.rand() < prob:
        ...
        volume[:, cut_y : cut_y + cut_h, cut_x : cut_x + cut_w] = 0
...
class InkDetResNet3dCSNModel(nn.Module):
    ...
    def forward_image_feats(self, img):
        ...
        # numpy.amax(a, axis=None, out=None, ...) => return the maximum of an array or maximum along an axis.
...
class InkDetLightningModel3d(pl.LightningModule):
    ...
    def add_model_specific_args(
        ...
        parser.add_argument(
            ...
            type=str,
            metavar="DE",
            help="Depth of the feature extractor",
            dest="depth",
        )
        parser.add_argument(
            "--bottleneck_mode",
            ...
            type=str,
            metavar="BM",
            help="resnetcsn of bottleneck mode",
...
def get_args() -> argparse.Namespace:
    ...
    parser = InkDetLightningModel3d.add_model_specific_args(parent_parser)
